...
public class JavaModelOutput {
   ...
   *          the ClassDescriptor
   * @param shadow
   *          whether to generate the shadow class of an interface
  ...
  protected String generate(ClassDescriptor cld, boolean shadow) {
    ...
    if ((!cld.isInterface()) || shadow) {
      ...
      sb.append("import org.intermine.model.InterMineId;" + ENDL);
      sb.append(
          "import org.intermine.objectstore.intermine.NotXmlParser;" + ENDL);
      sb.append(
          "import org.intermine.objectstore.intermine.NotXmlRenderer;" + ENDL);
      if (hasCollections) {
        sb.append(
            "import org.intermine.objectstore.proxy.ProxyCollection;" + ENDL);
      }
      if (hasReferences) {
        sb.append(
            "import org.intermine.objectstore.proxy.ProxyReference;" + ENDL);
      }
    ...
    if (shadow) {
      sb.append(" implements ").append(TypeUtil.unqualifiedName(cld.getName()))
          .append(", ShadowClass");
    } else {
      if (!cld.isInterface()) {
    ...
    if (shadow) {
      sb.append(INDENT).append("public static final Class<")
          .append(TypeUtil.unqualifiedName(cld.getName()))
          .append("> shadowOf = ")
          .append(TypeUtil.unqualifiedName(cld.getName()))
          .append(".class;" + ENDL);
    ...
    if (cld.isInterface() && (!shadow)) {
      ...
      sb.append(generateFieldDescriptors(cld, true)).append(generateEquals(cld))
          .append(generateHashCode(cld)).append(generateToString(cld))
          .append(generateGetFieldValue(cld, false))
          .append(generateGetFieldValue(cld, true))
          .append(generateSetFieldValue(cld)).append(generateGetFieldType(cld));
      if (cld.getSuperDescriptors().size() > 0) {
        sb.append(generateGetObject(cld)).append(generateSetObject(cld))
   ...
   *
   * @param cld
   *          the ClassDescriptor of the class
   * @param supers
   *          true if go up the inheritence tree and output fields
   * @return the generated String
   */
  protected String generateFieldDescriptors(ClassDescriptor cld,
      boolean supers) {
   ...
   * @param field
   *          true if the class should have the associated field, or false if
   *          the field is in the superclass
   * @return java code
   */
  protected String generate(AttributeDescriptor attr, boolean field) {
    ...
    if (field) {
      sb.append(INDENT + "// Attr: " + attr.getClassDescriptor().getName() + "."
          + attr.getName() + ENDL).append(INDENT + "protected ")
          .append(attr.getType()).append(" ").append(attr.getName())
          .append(";" + ENDL);
    }
  ...
  protected String generate(ReferenceDescriptor ref, boolean field) {
    ...
    if (field) {
      sb.append(INDENT + "// Ref: " + ref.getClassDescriptor().getName() + "."
          + ref.getName() + ENDL).append(INDENT)
          .append("protected org.intermine.model.InterMineObject ")
          .append(ref.getName()).append(";" + ENDL);
    }
    sb.append(generateGetSet(ref, field)).append(ENDL);
    return sb.toString();
  }
  ...
  protected String generate(CollectionDescriptor col, boolean field) {
    ...
    if (field) {
      sb.append(INDENT + "// Col: " + col.getClassDescriptor().getName() + "."
          + col.getName() + ENDL).append(INDENT).append("protected ")
          .append(type).append(" ").append(col.getName()).append(" = new ")
          .append(impl).append("();" + ENDL);
    }
   ...
   * @param fieldPresent
   *          true if this class has the associated field
  ...
  protected String generateGetSet(FieldDescriptor field, boolean fieldPresent) {
    ...
    sb.append(INDENT).append("public ").append(type).append(" get")
        .append(StringUtil.reverseCapitalisation(name)).append("()");
    if (!fieldPresent) {
      sb.append(";" + ENDL);
    } else {
      sb.append(" { ");
      if ((field instanceof ReferenceDescriptor)
        ...
        sb.append("if (").append(name)
            .append(
                " instanceof org.intermine.objectstore.proxy.ProxyReference) { return ")
            .append("((").append(type)
            .append(") ((org.intermine.objectstore.proxy.ProxyReference) ")
            .append(name).append(").getObject()); }; return (").append(type)
            .append(") ").append(name).append("; }" + ENDL);
      } else {
    ...
    // Set method
    sb.append(INDENT).append("public void ").append("set")
        .append(StringUtil.reverseCapitalisation(name)).append("(final ")
        .append(type).append(" ").append(name).append(")");
    if (!fieldPresent) {
      sb.append(";" + ENDL);
    } else {
      sb.append(" { ").append("this.").append(name).append(" = ").append(name)
          .append("; }" + ENDL);
    }

    if (field instanceof ReferenceDescriptor) {
      if (field instanceof CollectionDescriptor) {
        sb.append(INDENT).append("public void add")
            .append(StringUtil.reverseCapitalisation(name)).append("(final ")
            .append(((CollectionDescriptor) field)
   ...
   * 
   * @param cld
   *          descriptor for class in question
  ...
  protected String generateEquals(ClassDescriptor cld) {
    if (cld.getFieldDescriptorByName("id") != null) {
      ...
      sb.append(INDENT)
          ...
          .append(unqualifiedName).append(" && id != null) ? id.equals(((")
          .append(unqualifiedName).append(")o).getId()) : this == o; }" + ENDL);
      return sb.toString();
    } else {
      return "";
    }
  ...
  protected String generateToString(ClassDescriptor cld) {
    ...
    if (keyFields.size() > 0) {
      sb.append(INDENT).append("@Override public String toString() { ")
          .append("return \"").append(unqualifiedName).append(" [");
   ...
   * 
   * @param field
   *          descriptor for the field in question
  ...
  protected String getType(FieldDescriptor field) {
    ...
    if (field instanceof AttributeDescriptor) {
      ...
      type = ((ReferenceDescriptor) field).getReferencedClassDescriptor()
          .getName();
    }
    return type;
  }
      ...
      if (field instanceof AttributeDescriptor) {
        ...
        if (attribute.getType().startsWith("java.")) {
          sb.append(INDENT + INDENT)
              ...
              .append(INDENT + INDENT + INDENT).append("sb.append(\"" + DELIM
                  + "a" + field.getName() + DELIM + "\")");
        ...
        } else if ("org.intermine.objectstore.query.ClobAccess"
          ...
          sb.append(INDENT)
              .append(INDENT).append("sb.append(\"" + DELIM + "a"
                  + field.getName() + DELIM + "\")")
        ...
        sb.append(INDENT + INDENT)
            ...
            .append(INDENT + INDENT + INDENT)
            .append(
                "sb.append(\"" + DELIM + "r" + field.getName() + DELIM + "\")")
  ...
  public String generateSetObject(ClassDescriptor cld) {
    ...
    for (FieldDescriptor field : cld.getAllFieldDescriptors()) {
      ...
      if (field instanceof AttributeDescriptor) {
        ...
        if ("boolean".equals(attribute.getType())) {
          ...
          if ("org.intermine.model.InterMineId".equals(attribute.getType())) {
          sb.append(fieldName + " = InterMineId.valueOf(notXml[i]);\n");
        } else if ("InterMineId".equals(attribute.getType())) {
          sb.append(fieldName + " = InterMineId.valueOf(notXml[i]);\n");
          ...
          sb.append("StringBuilder string = null;\n")
              ...
              .append(INDENT + INDENT + INDENT + INDENT).append("}\n")
              .append(INDENT + INDENT + INDENT + INDENT).append(fieldName
                  + " = string == null ? notXml[i] : string.toString();\n");
        } else {
          throw new IllegalArgumentException(
              "Unknown type " + attribute.getType());
        }
        sb.append(INDENT + INDENT + INDENT + INDENT).append("i++;\n")
            .append(INDENT + INDENT + INDENT).append("}\n");
      } else if (field.isReference()) {
        ...
        sb.append(INDENT + INDENT + INDENT)
            ...
            .append(INDENT + INDENT + INDENT + INDENT)
            .append(fieldName
                + " = new ProxyReference(os, InterMineId.valueOf(notXml[i])"
    ...
    for (FieldDescriptor field : cld.getAllFieldDescriptors()) {
      ...
      if (field instanceof CollectionDescriptor) {
        ...
        sb.append(INDENT + INDENT)
            .append(fieldName + " = new ProxyCollection<"
                + coll.getReferencedClassName() + ">(os, this, \"" + fieldName
   ...
   *          the ClassDescriptor
   * @param proxy
   *          false to make the getFieldValue method, true to make the
   *          getFieldProxy method
      ...
      if (field instanceof AttributeDescriptor) {
        ...
        if (proxy) {
          ...
          sb.append("if (" + fieldName + " instanceof ProxyReference) {\n")
              .append(INDENT + INDENT + INDENT + INDENT)
              .append(
                  "return ((ProxyReference) " + fieldName + ").getObject();\n")
    ...
    for (FieldDescriptor field : cld.getAllFieldDescriptors()) {
      ...
      if (field instanceof AttributeDescriptor) {
        ...
        if ("boolean".equals(attribute.getType())) {
          ...
          sb.append(INDENT + INDENT + INDENT)
              .append(fieldName + " = ((InterMineId) value).intValue();\n");
        } else if ("long".equals(attribute.getType())) {
          sb.append(INDENT + INDENT + INDENT)
        ...
        sb.append(INDENT + INDENT + INDENT).append(
            fieldName + " = (org.intermine.model.InterMineObject) value;\n");
      } else {
        sb.append(INDENT + INDENT + INDENT)
      ...
      if (field.isCollection()) {
        ...
        sb.append("if (\"" + field.getName() + "\".equals(fieldName)) {\n")
            ...
            .append(fieldName + ".add(("
                + ((CollectionDescriptor) field).getReferencedClassName()
                + ") element);\n")
            .append(INDENT + INDENT).append("} else ");
      }
    }
    sb.append("{\n").append(INDENT + INDENT + INDENT)
        .append("if (!" + cld.getName() + ".class.equals(getClass())) {\n")
  ...
  public String generateGetFieldType(ClassDescriptor cld) {
    ...
    for (FieldDescriptor field : cld.getAllFieldDescriptors()) {
      ...
      if (field instanceof AttributeDescriptor) {
        ...
        sb.append(
            "return " + ((ReferenceDescriptor) field).getReferencedClassName()
                + ".class;\n");
      } else {
    ...
    sb.append(INDENT + INDENT)
        .append("if (!" + cld.getName() + ".class.equals(getClass())) {\n")
        .append(INDENT + INDENT + INDENT)
        .append("return TypeUtil.getFieldType(" + cld.getName()
            + ".class, fieldName);\n")
        .append(INDENT + INDENT).append("}\n").append(INDENT + INDENT)
        .append(
            "throw new IllegalArgumentException(\"Unknown field \" + fieldName);\n")
  ...
  public String generateGetElementType(ClassDescriptor cld) {
    ...
    sb.append(INDENT + INDENT)
        .append("if (!" + cld.getName() + ".class.equals(getClass())) {\n")
        .append(INDENT + INDENT + INDENT)
        .append("return TypeUtil.getElementType(" + cld.getName()
            + ".class, fieldName);\n")
        .append(INDENT + INDENT).append("}\n").append(INDENT + INDENT)
        .append(
            "throw new IllegalArgumentException(\"Unknown field \" + fieldName);\n")
