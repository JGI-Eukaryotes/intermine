...
public class GenerateUpdateTriggersTask extends Task
{
    ...
    public final void execute() {
        ...
        try {
            ...
            String keyCheckFileName = "foreign-key-checker.sql";
    ...
    private static String getRemoveSequence() {
        ...
        return "DROP SEQUENCE im_post_build_insert_serial;\n"
            + "SELECT setval('serial',(select (max(max)-1)/1000000 "
            + "from (select max(id) from intermineobject union "
            + "select (max(id)+2::bigint^32)::bigint%(2::bigint^32)::bigint from intermineobject where id < 0 )_z));\n";
    }
}
